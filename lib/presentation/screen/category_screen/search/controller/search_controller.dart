...
class SearchController extends GetxController {
  ...
  /// Loading state — shimmer shows when true
  ...
  Future<void> fetchProviders() async {
    try {
      isLoading.value = true; // ✅ shimmer starts
      ...
      isLoading.value = false; // ✅ shimmer ends (always, even on error)
    }
  }
  ...
  Future<bool> addFavorite(String providerId) async {
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        Get.snackbar(
          "Error",
          response.body['message'] ?? "Failed to add favorite",
        );
        return false;
      }
    } catch (e) {
